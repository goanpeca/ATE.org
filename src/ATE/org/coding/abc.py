...
class testABC(abc.ABC):
    ...
    def __init__(self, call_values=None, limits=None, SBINs=None, TNUMS=None):
    ...
    def __call__(self, call_values=None, limits=None):
    ...
    def randn(self, parameter, μ=None, σ=None):
        """draws a normal distributed random number for 'parameter'.

        parameter is one of the input parameters, if not, μ = 0.0 & σ = 1.0
        If μ is None : use the Nominal value of the input parameter
        If μ is not None : use that value
        If σ is None : use the biggest distance to LTL/UTL as the 3σ value
        If σ is not None : use that value
        """
        if parameter not in self._input_parameters:
            if μ is None:
                μ = 0.0
            if σ is None:
                σ = 1.0
            ...
            if μ is None:
                μ = self._input_parameters[parameter]['Nom']
            if σ is None:
                if np.isnan(self._input_parameters[parameter]['LTL']):
                    LTL = self._input_parameters[parameter]['LSL']
                else:
                    LTL = self._input_parameters[parameter]['LTL']

                if np.isnan(self._input_parameters[parameter]['UTL']):
                    UTL = self._input_parameters[parameter]['USL']
                else:
    ...
    def _extract_tester(self):
        ...
        # on the other hand ... the tester is defined in HW/__init__.py !
        return retval
    ...
    def _get_targets(self):
        ...
        if 'do' not in members_of_interest:

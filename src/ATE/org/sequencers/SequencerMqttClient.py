...
from ATE.org.sequencers import Harness
...
class SequencerMqttClient(Harness.Harness):
    ...
    def __init__(self, params: TheTestAppParameters = None, sequencer_instance=None):
    ...
    @classmethod
    def run_from_command_line_with_sequencer(cls, argv, client_instance, sequencer_instance=None):
        params = cls.init_from_command_line(argv)
        if sequencer_instance is None:
            from ATE.org.sequencers import SequencerBase
            sequencer_instance = SequencerBase(None)
        client_instance.apply_parameters(params, sequencer_instance)
        client_instance.run()

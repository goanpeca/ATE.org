...
# -*- coding: utf-8 -*-
"""
Created on Sat May 23 12:33:16 2020
...
class STDR(ABC):
    ...
    def set_value(self, FieldID, Value):
        ...
        if isinstance(FieldID, int):
            for field in self.fields:
                if self.fields[field]['#'] == FieldID:
                    FieldKey = field
            if FieldKey == '':
                raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{FieldID}' is not a valid key")
            ...
            raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) : Error : '{Value}' is not a string or integer.")
                ...
                if Bytes == '1':
                    for index in range(len(Value)):
                        if isinstance(Value[index], int):
                            if Value[index] < 0:
                                temp[index] = 0
                            elif Value[index] > 255:
                                temp[index] = 255
                            else:
                                temp[index] = Value[index]
                        elif isinstance(Value[index], float):
                            raise STDFError("not yet implemented")
                        else:
        ...
        # xN --> list of lists of nibble integers
                ...
                if self.local_debug:
                    print("%s._set_value(%s, %s) -> Value = %s, Reference '%s' = %s" % (self.id, FieldKey, Value, temp, Ref, len(temp)))
        # xV --> list of tuple (type, value) where type is defined in spec page 62
            elif Type == 'xV':
                """
                ...
                """
                if Bytes.isdigit():
                    raise STDFError("{self.id}.set_value({FieldID}, {Value}) : Unimplemented type '{'*'.join((Type, Bytes))}'")
                elif Bytes == 'n':
                    raise STDFError("%s.set_value(%s, %s) : Unimplemented type '%s'" % (self.id, FieldKey, Value, str(K) + '*'.join((Type, Bytes))))
                elif Bytes == 'f':
                    raise STDFError("%s.set_value(%s, %s) : Unimplemented type '%s'" % (self.id, FieldKey, Value, str(K) + '*'.join((Type, Bytes))))
                ...
                # assign from temp to field
                if self.local_debug:
                    print(f"{self.id}._set_value({FieldID}, {Value}) -> Value = {temp}, Reference '{Ref}' = {len(temp)}")
            else:
                raise STDFError("%s.set_value(%s, %s) Error : '%s' is an unsupported Type" % (self.id, FieldKey, Value, '*'.join((Type, Bytes))))
        else:
            temp = ''
        # U --> unsigned integer ✔
            if Type == 'U':
                if not isinstance(Value, int):
                    raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{Value}' is not a an integer")
                if Bytes == '1':
                    if ((Value >= 0) and (Value <= 255)):
                        temp = Value
                    else:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{Value}' can not be casted into U*1")
                elif Bytes == '2':
                    if ((Value >= 0) and (Value <= 65535)):
                        temp = Value
                    else:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{Value}' can not be casted into U*2")
                elif Bytes == '4':
                    if ((Value >= 0) and (Value <= 4294967295)):
                        temp = Value
                    else:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{Value}' can not be casted into U*4")
                elif Bytes == '8':
                    if ((Value >= 0) and (Value <= 18446744073709551615)):
                        temp = Value
                    else:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{Value}' can not be casted into U*8")
                else:
                    raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{'*'.join((Type, Bytes))}' is an unsupported Type")
                self.fields[FieldKey]['Value'] = temp
                if self.local_debug:
                    print(f"{self.id}._set_value({FieldID}, {Value}) -> Value = {temp}")
        # I --> signed integer ✔
            elif Type == 'I':
                if not isinstance(Value, int):
                    raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) : '{Value}' is not an integer")
                if Bytes == '1':
                    if ((Value >= -128) and (Value <= 127)):
                        temp = Value
                    else:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) : '{Value}' can not be casted into I*1")
                elif Bytes == '2':
                    if ((Value >= -32768) and (Value <= 32767)):
                        temp = Value
                    else:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) : '{Value}' can not be casted into I*2")
                elif Bytes == '4':
                    if ((Value >= -2147483648) and (Value <= 2147483647)):
                        temp = Value
                    else:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) : '{Value}' can not be casted into I*4")
                elif Bytes == '8':
                    if ((Value >= -36028797018963968) and (Value <= 36028797018963967)):
                        temp = Value
                    else:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) : '{Value}' can not be casted into I*8")
                else:
                    raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) : Unsupported type '{'*'.join((Type, Bytes))}'")
                self.fields[FieldKey]['Value'] = temp
                if self.local_debug:
                    print("{self.id}._set_value({FieldID}, {Value}) -> Value = {temp}" % (self.id, FieldKey, Value, temp))
        # R --> float ✔
            elif Type == 'R':
                if not isinstance(Value, (int, float)):
                    raise STDFError("{self.id}.set_value({FieldID}, {Value}) : '{Value}' is not of type float or int")
                if ((Bytes == '4') or (Bytes == '8')):
                    temp = float(Value)
                else:
                    raise STDFError("{self.id}.set_value({FieldID}, {value}) : Unsupported type '{'*'.join((Type, Bytes))}'")
                self.fields[FieldKey]['Value'] = temp
                if self.local_debug:
                    print("{self.id}._set_value({FieldID}, {Value}) -> Value = '{temp}'")
        # C --> string ✔
            elif Type == 'C':
                if not isinstance(Value, (str, bytes)):
                    raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{Value}' is not of type str or bytes")
                if isinstance(Value, bytes):
                    try:
                        Value = Value.decode('UTF8')
                    except Exception:
                        raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) Error : '{Value}' is of type bytes, but can not be decode as 'UTF-8'")
                if Bytes.isdigit():
                    temp = Value.strip()[:int(Bytes)]  # loose pre- and post string whitespaces and limit to max size
                    if len(temp) < int(Bytes):  # string is too small
                        temp = temp.ljust(int(Bytes), " ")  # adjust the size (pad left with spaces)
                elif Bytes == 'n':
                    temp = Value.strip()[:255]  # loose pre- and post string whitespaces and limit to max size
                else:
                    raise STDFError(f"{self.id}.set_value({FieldID}, {Value}) : Unsupported type '{'*'.join((Type, Bytes))}'")
                self.fields[FieldKey]['Value'] = temp
                if self.local_debug:
        ...
        if Type == 'U':  # (list of) Unsigned integer(s)
            ...
            if Size.isdigit():
                if Size == '1':
                    ...
                    if TypeMultiplier:
                        raise STDFError("%s._pack_item(%s) : Unsupported type-format '%s'" % (self.id, FieldKey, str(K) + TypeFormat))
                    else:
                        raise STDFError("%s._pack_item(%s) : Unsupported type-format '%s'" % (self.id, FieldKey, TypeFormat))
            else:
                if TypeMultiplier:
                    raise STDFError("%s._pack_item(%s) : Unsupported type-format '%s'" % (self.id, FieldKey, str(K) + TypeFormat))
                else:
            ...
            if self.local_debug:
                if TypeMultiplier:
                    print(f"{self.id}._pack_item({FieldKey})")
    ...
    def __bytes__(self):
        """This method will create the byte representation of the record.

        Note: This is the record as it can be found **IN** the STDF file.
        """
        sequence = {}
        header = b''
        body = b''
        ...
        if self.get_fields('REC_LEN')[3] != len(body):
            raise STDFError("{self.id}.__bytes__() length error {self.get_fields('REC_LEN')[3]} != {len(body)}")
        ...
        if self.local_debug:
            print("{self.id}.__bytes__() --> '{hexify(retval)}' {len(retval)} bytes")
        return retval

    def __repr__(self):
        """This method will return the object representation."""
        return f"{self.id}({self.endian}, '{self.__bytes__()}')"

    def __str__(self):
        """This method retruns the human readable version of this record."""
        time_fields = ['MOD_TIM', 'SETUP_T', 'START_T', 'FINISH_T']
        sequence = {}
        for field in self.fields:
            sequence[self.fields[field]['#']] = field
        retval = f"   {self.id} ({self.get_value('REC_TYP')}, {self.get_value('REC_SUB')}) @ {self.version}\n"
        for field in sorted(sequence):
            retval += f"      {sequence[field]} = '{self.fields[sequence[field]]['Value']}'"
            retval += f" [{self.fields[sequence[field]]['Type']}] ({self.fields[sequence[field]]['Text'].strip()})"
            if self.fields[sequence[field]]['Ref'] is not None:
                retval += f" -> {self.fields[sequence[field]]['Ref']}"
